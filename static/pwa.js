(() => {
  ...
  button.type = "button";
  button.textContent = "Preparing Notes…";
  ...
  Object.assign(button.style, {
    ...
    zIndex: "9999", display: "block", padding: "11px 15px", border: "1px solid #8b6cff",
  ...
  async function prepare() {
    if (!("serviceWorker" in navigator) || !window.isSecureContext) {
      button.textContent = "Check Notes install";
      return;
    }
    try {
    ...
    setTimeout(() => {
      if (!promptEvent) button.textContent = "Check Notes install";
    }, 1500);
  }
